...
* **`background` — the `-solid` state (always dark, one per role):** start from the palette's own anchor step (where the user's input hex landed, per Tier 1) and move toward **darker** steps only as far as needed; pick the first step that clears WCAG AA against whatever text sits on it (per `SKILL.md` Phase 3). Never search past that point, and never pick a step lighter than the anchor. If the anchor already passes (e.g. `brand-500` when the input hex is dark/saturated enough), `-solid` uses it directly; otherwise keep stepping darker (typically `700`–`900`). `-solid` is always exactly one alias per role, and is always the darkest token that role owns.
